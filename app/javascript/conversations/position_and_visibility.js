...
function positionChatWindows() {
    ...
    for (let i = 0; i < chat_windows_count; i++) {
        const right_position = i * 410;
        const chat_window = i + 1;
...
function hideShowChatWindow() {
    ...
    let $conversation_window = $('.conversation-window:nth-of-type(' + gon.last_visible_chat_window + ')');
    // get an offset of the most left conversation window
    const offset = $conversation_window.offset();
    ...
    if (offset.left < 50 && gon.last_visible_chat_window !== 1) {
        $conversation_window.css('display', 'none');
        gon.hidden_chats++;
    ...
    if (offset.left > 550 && gon.hidden_chats !== 0) {
        gon.hidden_chats--;
        gon.last_visible_chat_window++;
        $conversation_window.css('display', 'initial');
    }
}